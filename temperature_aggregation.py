...
RASTER_DIR = Path("data/ERA5_Temperature")
CATCHMENTS_PATH = Path("data/Ancillary/global_catchments_cci/global_catchments_cci.shp")
OUTPUT_PATH = Path("backend/outputs/temperature_timeseries.csv")
...
def main():
    print("Starting temperature aggregation...")
    ...
    print(f"Number of ERA5 temperature raster files: {len(raster_files)}")
                    ...
                    try:
                        ...
                        if band.size != 0:
                            ...
                            if finite_values.size != 0:
                                # The provided raster already stores mean air temperature in °C.
                                temperature = float(np.nanmean(finite_values))
                    except ValueError:
                        temperature = np.nan
